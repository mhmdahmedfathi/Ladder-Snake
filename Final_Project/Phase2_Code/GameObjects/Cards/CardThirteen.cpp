...
bool CardThirteen::Exists13 = 0;
bool CardThirteen::isBought = 0;
int CardThirteen::CardPrice = 0;
int CardThirteen::Fees = 0;
Player* CardThirteen::cardOwner = NULL;
int CardThirteen::Only1Time = 0;
int CardThirteen::Only1TimeLoad = 0;
...
void CardThirteen::ReadCardParameters(Grid * pGrid)
{
	...
	//a Static data member to make sure that the parameters of cardThirteen has been entered once
	...
	pOut->PrintMessage("New CardThirteen: Enter the Card Price to be paid by the cardOwner ...");
...
void CardThirteen::EditCardParameters(Grid * pGrid)
{
	...
	//Reading an Integer from the user using the Input class and set the Price and the Fees parameters with it
	pOut->PrintMessage("Editing CardThirteen: Enter the new Card Price to be paid by the cardOwner ...");
	int unValidatedPrice = pIn->GetInteger(pOut);
...
void CardThirteen::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (!isBought)
	{
		...
		if ((pPlayer->GetWallet()) >= CardPrice)
		{
			...
			pOut->PrintMessage("You have reached CardThirteen, card's Price= " + to_string(CardPrice) + " Press 'Y' to buy it or 'N' to skip");
