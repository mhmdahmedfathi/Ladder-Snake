...
int CardTwelve::Only1Time = 0;
int CardTwelve::Only1TimeLoad = 0;
...
void CardTwelve::EditCardParameters(Grid * pGrid)
{
	...
	//Reading an Integer from the user using the Input class and set the Price and the Fees parameters with it
	pOut->PrintMessage("Editing CardTwelve: Enter the new Card Price to be paid by the cardOwner ...");
	int unValidatedPrice = pIn->GetInteger(pOut);
...
void CardTwelve::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (!isBought)
	{
		...
		if ((pPlayer->GetWallet()) >= CardPrice)
		{
			...
			pOut->PrintMessage("You have reached CardTwelve, card's Price= " + to_string(CardPrice) + " Press 'Y' to buy it or 'N' to skip");
